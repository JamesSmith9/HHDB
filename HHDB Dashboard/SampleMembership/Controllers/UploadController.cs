...
namespace SampleMembership.Controllers
{
	...
	public class UploadController : Controller
	{
		...
		[HttpPost]
		public ActionResult ImportForm([Bind(Include = "SurveyID")]Survey model, int[] yes, int[] no, int[] one, int[] two, int[] three, int[] four, int[] five)
		{

			model.SurveyQuestions = db.SurveyXQuestions.Where(x => x.SurveyID == model.SurveyID)
				.Select(x => x.Question).ToList();

			model.SXQIDs = db.SurveyXQuestions.Where(x => x.SurveyID == model.SurveyID)
				.Select(x => x.SXQID).ToList();
